...
example (a b c : ℝ) (h : a = -b) (h' : b + c = 0) : b*(a - c) = 0 := by
  calc
    b*(a - c) = b*((-b) - c) := by congr; apply h
    _ = b*(-(b + c)) := by ring
    _ = b*(-0) := by congr; apply h'
    _ = 0 := by ring
...
lemma uniq_limit (hl : seq_limit u l) (hl' : seq_limit u l') : l = l' := by
  ...
  rcases hl' ε ε_pos with ⟨N₂, hN₂⟩
  let N := max N₁ N₂
  have h1 := hN₁ N (by apply le_max_left)
  have h2 := hN₂ N (by apply le_max_right)
  calc
...
lemma subseq_tendsto_of_tendsto' (h : seq_limit u l) (hφ : extraction φ) :
  ...
  intro n hn
  apply hN
  exact le_trans (id_le_extraction' hφ n) hn

/-- If `u` tends to `l` all its cluster points are equal to `l`. -/
lemma cluster_limit (hl : seq_limit u l) (ha : cluster_point u a) : a = l := by
  rcases ha with ⟨φ, hφ, hlim⟩
  have h1 := subseq_tendsto_of_tendsto' hl hφ
  exact uniq_limit h1 hlim
...
example : (∃ l, seq_limit u l) → CauchySequence u := by
  ...
  calc
    ...
    _ ≤ |u p - l| + |u q - l| := by apply abs_sub_le
    _ ≤ ε/2 + ε/2 := by gcongr
    _ = ε := by ring
